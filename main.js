...
const modalContainerElement = document.createElement('span');
modalContainerElement.classList.add('fimanager', 'x_shift', 'flaticon-030-shift');
modalContent.appendChild(modalContainerElement);

// textarea 
const textarea = document.createElement('textarea');
textarea.classList.add('output-window');
textarea.setAttribute('placeholder', 'Text');
document.body.appendChild(textarea);

// log
const logsContainer = document.createElement('div');
logsContainer.classList.add('logs');
document.body.appendChild(logsContainer);

const logs = ['count_signs', 'event_code', 'event_which', 'event_location', 'event_key'];

for (let i = 0; i < logs.length; i++) {
  const log = document.createElement('input');
  log.classList.add('event', logs[i]);
...
speechContainer.classList.add('speech');
document.body.appendChild(speechContainer);
const speechContent = document.createElement('span');
speechContent.classList.add('speech-to-text');
speechContainer.appendChild(speechContent);
